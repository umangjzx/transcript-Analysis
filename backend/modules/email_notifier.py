...
Configuration (via .env or environment variables):
    ...
    SMTP_FROM_NAME   Display name                  (default: AuraSafety)
...
def _alert_html(
    ...
    inner = (
        _header_row(f"🚨 Safety Alert — {severity.upper()} Risk",
                    f"AuraSafety · {datetime.now().strftime('%B %d, %Y at %H:%M')}",
                    color)
        ...
        + _footer_row("This alert was generated automatically by AuraSafety · Do not reply")
    )
    return _wrap(inner)
...
def _summary_html(
    ...
    inner = (
        _header_row(f"📋 Analysis Summary",
                    f"{filename} · AuraSafety · {datetime.now().strftime('%B %d, %Y at %H:%M')}",
                    "#6366f1")
        + _score_info_row(filename, severity, risk_score, report_id, color, sc)
        ...
        + _footer_row(f"Generated by AuraSafety · Report #{report_id} · {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    )
    return _wrap(inner)
